        ...
        <div className="flex gap-2">
          ...
          {onScanBarcode && (
            <button
              ...
              className="flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-lg border border-surface-border text-secondary hover:border-brand hover:text-brand transition-colors"
      ...
      <div className="flex-1 overflow-y-auto pb-24">
        {loading && (
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-[13px] text-secondary">Searching…</p>
          </div>
        )}
        ...
        {!loading && !error && query && results.length === 0 && (
          <div className="px-4 py-10 text-center">
            <p className="text-[13px] text-secondary">No results for &ldquo;{query}&rdquo;</p>
          </div>
        )}

        {!loading && !error && !query && (
          <div className="px-4 py-10 text-center">
            <Search size={32} className="mx-auto mb-3 text-progress-track" />
            <p className="text-[13px] text-secondary">Search the Open Food Facts database</p>
          </div>
        )}
        ...
        {!loading && results.map(product => {
          ...
          return (
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-3.5 border-b border-surface-border hover:bg-background text-left transition-colors"
            >
              <div className="flex-1 min-w-0">
                <p className="text-[15px] font-semibold text-section-header leading-snug truncate">{name}</p>
                {product.brands && (
                  <p className="text-[13px] text-secondary mt-0.5 truncate">{product.brands}</p>
                )}
                <p className="text-[13px] text-secondary mt-0.5">Per 100g</p>
              </div>
              <div className="flex-shrink-0 text-right">
                <p className="text-[15px] font-bold text-brand">{Math.round(per100.calories)}</p>
                <p className="text-[13px] text-secondary">kcal</p>
              </div>
            </button>
          );
        })}
